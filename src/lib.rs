pub mod elliptic_curve {
    ...
    mod tests {
        ...
        fn test_point_equality() {
            ...
            let p1 = Pair { x: BigUint::new(vec![0]), y: BigUint::new(vec![0]) };
            assert!(p1 != p2);
            let p2 = Pair { x: BigUint::new(vec![0]), y: BigUint::new(vec![0]) };
            assert_eq!(p1, p2);

            let p1 = Pair { x: BigUint::new(vec![1]), y: BigUint::new(vec![0]) };
            assert!(p1 != p2);
            assert!(p1 != Inf);
        }

        #[test]
        fn test_elliptic_cuve_constructor() {
            // produce toy elliptic curve
            let a = BigUint::new(vec!(1));
            let b = BigUint::new(vec!(6));
            let g = Pair {x: BigUint::new(vec!(2)), y: BigUint::new(vec!(4)) };
            let p = BigUint::new(vec!(11));
            let curve1 = EllipticCurve::new(a, b, g, p);
